...
async function addRelatoriosColumns() {
  ...
  try {
    ...
    await sql`
      ...
      ADD COLUMN IF NOT EXISTS orgao_atendido VARCHAR(255),
      ...
      ADD COLUMN IF NOT EXISTS numero_autenticacao VARCHAR(50) UNIQUE
    `
    console.log("Colunas adicionadas à tabela relatorios_visitas")
    ...
    `
    console.log("Tabela relatorios_anexos criada")
    ...
    `
    console.log("Índice criado para numero_autenticacao")
    ...
    console.log(`Atualizados ${relatoriosSemNumero.length} relatórios com número de autenticação`)
