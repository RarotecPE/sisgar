...
// GET - Buscar módulos de um órgão
export async function GET(
  ...
  try {
    ...
    const modulos = await sql`
      ...
      FROM orgaos_modulos
      WHERE orgao_id = ${parseInt(id)}
      ORDER BY created_at DESC
    `
    
    return NextResponse.json(modulos)
  } catch (error) {
    console.error("Erro ao buscar módulos do órgão:", error)
    return NextResponse.json({ error: "Erro ao buscar módulos" }, { status: 500 })
  }
}

// POST - Adicionar módulos a um órgão
export async function POST(
  ...
  try {
    ...
    const orgaoId = parseInt(id)
    const resultados = []
    
    for (const modulo of modulos) {
      try {
        const result = await sql`
          INSERT INTO orgaos_modulos (orgao_id, modulo, adicionado_por, adicionado_por_nome)
          VALUES (${orgaoId}, ${modulo}, ${session.id}, ${session.nome})
...
// DELETE - Remover módulo de um órgão
export async function DELETE(
  ...
  try {
    ...
    await sql`
      DELETE FROM orgaos_modulos
      WHERE orgao_id = ${parseInt(id)} AND modulo = ${modulo}
    `
