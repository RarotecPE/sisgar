-- Migration 008: Gestão Técnica (Responsabilidades, Checklists) e Capacitação/Tutoriais
BEGIN;

-- 1. Responsabilidades por Módulo
...
-- Importação / Conciliação de Responsáveis
CREATE TABLE IF NOT EXISTS responsaveis_importacao (
  ...
  status TEXT NOT NULL DEFAULT 'pendente'
    CHECK (status IN ('pendente', 'pronto', 'importado', 'ignorado', 'nao_aplicavel')),
  observacao TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Trigger de validação órgão x cliente
...
-- Seeds padrão do modelo de checklist
...
-- 3. Capacitações e Treinamentos
CREATE TABLE IF NOT EXISTS capacitacoes (
  id SERIAL PRIMARY KEY,
  titulo TEXT NOT NULL,
  data_treinamento DATE NOT NULL,
  instrutores JSONB NOT NULL DEFAULT '[]'::jsonb,
...
CREATE INDEX IF NOT EXISTS idx_capacitacoes_data
  ON capacitacoes (data_treinamento DESC);

-- 4. Tutoriais (Documentos Institucionais)
CREATE TABLE IF NOT EXISTS tutoriais (
  id SERIAL PRIMARY KEY,
  titulo TEXT NOT NULL,
  data_tutorial DATE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_tutoriais_data
  ON tutoriais (data_tutorial DESC);
